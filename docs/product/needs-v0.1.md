...
| **技术栈**   | 保留 Nuxt + Vue 生态，V1 升级到最新版（TL 评估）；atomic CSS 引擎 = **Tailwind CSS v4**（DS-D-02 锁定）；ayingott.me 当前 UnoCSS 不动，待 `@ayingott/theme` V0 ready 后单独 PR 迁移 |
...
- Nuxt 3.x → 最新（含 Vue / Nitro / color-mode 同步升级）
- 当前 UnoCSS 保留；Tailwind v4 迁移**不在 S1 范围**，待 `@ayingott/theme` V0 ready 后单独 PR 处理
- ESLint / TS config 跟进
...
| **AY-D-05** | 保留 Nuxt + Vue 生态升级到最新版；atomic CSS 引擎 = Tailwind CSS v4（DS-D-02 锁）；ayingott.me 当前 UnoCSS 待 @ayingott/theme V0 后单独 PR 迁移 |
